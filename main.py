				...
				if interaction.content in DELETION:
					...
					if newhash == False:
						await interaction.author.send(
							"incorrect format ... or a bug? hash syntax: x label"+HASH_DELIM+" y label"
						)
...
@app_commands.describe(
	hash = "format 🌇"+HASH_DELIM+"🌄"+HASH_DELIM+"🌌"+HASH_DELIM+"🌃"+HASH_DELIM+"🌞"+HASH_DELIM+"🏞️"+HASH_DELIM+"🌜"+HASH_DELIM+"🏬"+HASH_DELIM+"🗺️"+HASH_DELIM+"⏱️"+HASH_DELIM+"🥱 makes the graph directly",
	_1 = "1st quadrant",
	_2 = "2nd quadrant",
	_3 = "3rd quadrant",
	_4 = "4th quadrant",
...
async def tbt(interaction:discord.Interaction,
	...
	try:
		...
		if hash == False:
			...
			hash = await amendhash(hash, HASH_DELIM.join(hash_params))
			await tofile(hash)
			# send to discord
			with open('diagram.png', 'rb') as f:
				await interaction.response.send_message(
